...
See the [Getting Started Guide](guide/) for a complete walkthrough. For YAML syntax details including custom tags and quoting rules, see the [YAML Reference](yaml-reference/). For common issues, see [Troubleshooting](troubleshooting/).

## Commands

| Command | Description |
|---------|-------------|
| [init](commands/init/) | Create a new StarForge project |
| [build](commands/build/) | Build disk images for a target |
| [write](commands/write/) | Write a built target to a storage device |
| [chroot](commands/chroot/) | Enter the built filesystem interactively |
| [run](commands/run/) | Boot a built target in QEMU |
| [export](commands/export/) | Export build artifacts as disk or partition images |
| [inspect](commands/inspect/) | Inspect the resolved build context for a target |
| [list](commands/list/) | List targets defined in the project |
| [status](commands/status/) | Show project info and build state |
| [clean](commands/clean/) | Remove build artifacts |
...
Actions are the building blocks of layers. Each step in a layer specifies an action and its configuration. See the [Actions Reference](actions/) for full details on every action.
...
| Document | Contents |
|----------|----------|
| [Getting Started Guide](guide/) | Complete walkthrough from project creation to deployment |
| [Actions Reference](actions/) | All actions, step fields, override semantics, and build phases |
| [YAML Reference](yaml-reference/) | Custom tags, quoting rules, INI field conversion, and common patterns |
