...
For details on YAML quoting rules and all custom tags, see the [YAML Reference](yaml-reference/).
...
Included files are resolved relative to the including file's directory and can be nested up to 10 levels deep. See the [Actions Reference](actions/#include--file-inclusion) for full details.
...
You can also create service units from inline definitions or layer files, and create drop-in overrides with `extends`. See the [systemd-service reference](actions/systemd-service/) for all modes.

Other systemd unit types have matching actions: [`systemd-mount`](actions/systemd-mount/), [`systemd-timer`](actions/systemd-timer/), [`systemd-socket`](actions/systemd-socket/), [`systemd-slice`](actions/systemd-slice/), and [`systemd-target`](actions/systemd-target/).
...
Available tags: `!append`, `!prepend`, `!before`, `!after`, `!truncate_before`, `!truncate_after`. See the [`file-edit` reference](actions/file-edit/) for all options.
...
Drop-in field names use `snake_case` and are converted to systemd's `CamelCase` automatically. See the [YAML Reference](yaml-reference/#systemd-ini-field-names) for the full conversion rules.
...
Includes can also reference URLs and nest up to 10 levels deep. See the [YAML Reference](yaml-reference/#include) for full details.
