...
Run a script on the host during the Collect phase to compute build variables dynamically. Unlike [`run`](run.md), which executes inside the target chroot during the build, `layer-run` executes on the host machine before any build phases begin.
...
- Scripts run on the host machine, not inside the target chroot. This is the key difference from [`run`](run.md), which executes in the chroot during phase 8.
...
- See [Variables](../concepts/variables.md) for the full variable system documentation.
