...
Copy files or directories within the target filesystem. This copies from one location in the target to another (not from the layer directory -- use [`file-create`](file-create.md) for that).
...
- To copy files from the layer directory into the target, use [`file-create`](file-create.md) with `layer_path`.
