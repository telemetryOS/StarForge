...
- The `mode` field must be a **quoted string** (e.g., `"0644"`, `"0755"`). Unquoted octal numbers like `0755` are interpreted by YAML as the integer 493. See the [YAML Reference](../yaml-reference.md#octal-numbers--always-quote-file-modes).
- Ownership of new files is inherited from the parent directory in the target filesystem. For example, a file created under `/home/player/` will be owned by `player:player` if that directory exists and is owned by that user. Use [`file-ownership`](file-ownership.md) to set explicit ownership.
- `file-create` does not have `owner` or `group` fields -- use [`file-ownership`](file-ownership.md) after creating the file.
