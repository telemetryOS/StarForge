...
The `starforge.yaml` defines your build targets -- each target references an ordered list of layers. See [Projects](concepts/projects.md) for details.
...
Each layer is a directory containing a `layer.yaml` file, plus any files that actions reference (configs, scripts, etc.). See [Layers](concepts/layers.md) for the full reference.
...
- [Guide](guide.md) -- In-depth walkthrough of layer writing, advanced features, and multi-target projects
- [Concepts](concepts/README.md) -- Architecture, build pipeline, caching, and override semantics
- [Actions Reference](actions/README.md) -- Complete reference for all actions
