...
- `sf_set` calls inside chroot scripts are no-ops. Variable output only works in [`layer-run`](../variables/) scripts, which execute on the host during the Collect phase.
